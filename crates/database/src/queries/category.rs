...
impl InsertStmt {
    pub async fn bind<'c, 'a, 's, C: GenericClient, T1: crate::StringSql, T2: crate::StringSql>(
        ...
        client.execute(stmt, &[name, description]).await
    }
}
    ...
    > for InsertStmt
{
    fn params(
        ...
        Box<dyn futures::Future<Output = Result<u64, tokio_postgres::Error>> + Send + 'a>,
    > {
        Box::pin(self.bind(client, &params.name, &params.description))
    }
}
pub fn get() -> GetStmt {
...
pub fn get_all() -> GetAllStmt {
    GetAllStmt(crate::client::async_::Stmt::new("SELECT * FROM categories"))
}
...
pub fn update() -> UpdateStmt {
    UpdateStmt(crate::client::async_::Stmt::new(
        "UPDATE categories SET name = COALESCE(name, $1), description = COALESCE(description, $2), update_at = now() WHERE id = $3",
    ))
}
